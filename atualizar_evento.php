...
if ($new_imagem_url !== null) {
    $stmt = $conn->prepare("UPDATE eventos SET nome=?, data=?, local=?, preco=?, descricao=?, imagemUrl=?, categoria=? WHERE id=?");
    $stmt->bind_param("sssssssi", $_POST['nome'], $data_mysql, $_POST['local'], $_POST['preco'], $_POST['descricao'], $new_imagem_url, $categoria, $id);
} else {
    $stmt = $conn->prepare("UPDATE eventos SET nome=?, data=?, local=?, preco=?, descricao=?, categoria=? WHERE id=?");
    $stmt->bind_param("ssssssi", $_POST['nome'], $data_mysql, $_POST['local'], $_POST['preco'], $_POST['descricao'], $categoria, $id);
}
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        ...
        echo json_encode(['success' => true, 'message' => 'Nenhum dado foi alterado.']);
    }
} else {
